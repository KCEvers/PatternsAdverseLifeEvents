...
prepare_data = function(name_dataset, filepath_base, event_dicts, rerun = F){
  ...
  dir.create(filepath_deriv, recursive = T, showWarnings = F)
  
  filepath_figs = file.path(filepath_base, "figs")
  dir.create(filepath_figs, showWarnings = F)
  
  filepath_figs_dataset = file.path(filepath_figs, name_dataset)
  dir.create(filepath_figs_dataset, showWarnings = F)
...
prepare_SHP = function(filepath_dataset, filepath_df_binary, event_dict, rerun){
  ...
  if (rerun | !file.exists(filepath_df_binary)){
    ...
    df_binary = df_raw %>%
      ...
      mutate(PL05 = ifelse(YEAR >= 2021, median(PL05A, PL05B, na.rm = T), PL05)) %>%
      ...
      mutate(wave_nr = as.numeric(as.character(YEAR)) - min(as.numeric(as.character(YEAR)), na.rm = T) + 1,
             ses = NA) %>%
...
prepare_HILDA = function(filepath_dataset, filepath_df_binary, event_dict, rerun){
  ...
  if (rerun | !file.exists(filepath_df_binary)){
    ...
    for (wave in letters[2:23]){ #  Exclude wave 1 which didn't measure life events
      ...
      df3 = merge(df1, df2, all.x = T) %>% mutate(wave = !!wave)
    ...
    # Merge dataframes
    df_binary = df_binary_ %>% merge(df_h_ids, all.x = T) %>%
      # Mutate age to numeric, haven-labelled otherwise
      mutate(age = as.numeric(as.character(factor(age))))
    df_binary %>% head
...
get_analysis_df = function(name_dataset, df_binary, event_dict, filepath_deriv, rerun){
  ...
  if (rerun | any(!sapply(c(filepath_df_per_event_pp_py,
    ...
    df_per_event_pp_py = df_binary %>%
      ...
      dplyr::summarise(nr_occur = sum(occurred == 1, na.rm = T),
                       nr_nooccur = sum(occurred == 0, na.rm = T),
      ...
      dplyr::mutate(
        first_obs_wave_nr_pp = min(wave_nr, na.rm  = T),
        last_obs_wave_nr_pp = max(wave_nr, na.rm  = T),
    ...
    test$p_id %>% unique() %>% length()
    test$crosswave_h_id %>% unique() %>% length()
    ...
    df_nr_negevents_pp = df_nr_negevents_pp_py %>%
      ...
      dplyr::summarise(nr_occur = sum(nr_occur, na.rm = T),
                       nr_nooccur = sum(nr_nooccur, na.rm = T),
                       nr_missing_occur = sum(nr_missing_occur, na.rm = T),
                       .groups = 'drop'
      )
