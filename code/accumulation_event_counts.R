...
run_accumulation = function(name_dataset, filepath_base, P, rerun = F){
  ...
  df_accum <<- datalist$df_nr_negevents_pp_py %>%
    ...
    arrange(p_id, t_id) %>%
    # Overwrite number of no occurrences to exclude missing data; missing = did not occur
    dplyr::mutate(nr_nooccur = n_trials - nr_occur,
                  # nr_missing = n_trials - (nr_occur + nr_nooccur_old)
                  )

  # Number of missing events in accumulation analysis
  table(df_accum$nr_missing_occur)
...
accumulation_table = function(SHP, HILDA){
  ...
  rbind(accumulation_create_df("SHP", SHP), accumulation_create_df("HILDA", HILDA)) %>% kbl(format = "latex",
                                                                                            escape=F,
                                                                                            caption = "Estimates of the Poisson, frailty, and Polya urn models in the accumulation analysis (profile 95\\% profile confidence intervals in brackets). Estimates are on the natural scale. Fit comparisons subtract the fit of the model on the right from that of the model on the left, such that a positive number indicates the model on the left fits better. $\\lambda$ = Yearly rate of adverse life events; $p$ = Per-trial probability of adverse life events; $\\phi$ = Dispersion; $i$ = Individual; $h$ = Household (Source: Swiss Household Panel, SHP and Household, Income and Labour Dynamics in Australia Survey, HILDA).",
                                                                                            label = "accumulation",
                                                                                            booktabs = TRUE,
                                                                                            col.names = NULL) %>%
    add_header_above(c(" ", "$\\\\lambda$" = 1, "$\\\\lambda$" = 1, "$i$" = 1, "$h$" = 1, "$\\\\Delta$AIC" = 1, "$\\\\Delta$BIC" = 1,
                     ...
                     escape = F) %>%
    add_header_above(c(" ", "Fixed" = 1, "Fixed" = 1, "Random $\\\\sigma$" = 2, "Fit (Poisson-Frailty)" = 2,  "Fixed" = 2, "Random $\\\\sigma$" = 2, "Fit (Frailty-Polya)" = 2),
                     escape = F)  %>%
